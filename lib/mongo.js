...
class MongoLib {
    ...
    getSearchDate(collection, SearchDate,SearchLocation) {
        return this.connect().then(db => {
            //if no location included, use query without location:
            if(!SearchLocation)
            {
            ...
            //if location included, use query with location (filtering at the end):
            else
            {
                return db.collection(collection).aggregate(
                    [
                        ...
                        },
                        {
                            $match: {
                                $expr: {
                                    $setIsSubset: [[`${SearchLocation}`], "$location"]
                                    }
                                }
                            },
                     ]).toArray();
